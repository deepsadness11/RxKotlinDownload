...
import cry.com.download.zlc.season.rxdownload2.entity.DownloadRange;
import cry.com.download.zlc.season.rxdownload2.entity.DownloadStatus;
import io.reactivex.FlowableEmitter;
import okhttp3.ResponseBody;
import retrofit2.Response;

import static cry.com.download.zlc.season.rxdownload2.function.Constant.CHUNKED_DOWNLOAD_HINT;
import static cry.com.download.zlc.season.rxdownload2.function.Utils.GMTToLong;
...
public class FileHelper {
    ...
    public void saveFile(FlowableEmitter<DownloadStatus> emitter, int i, File tempFile,
        ...
        Utils.log("Thread: " + Thread.currentThread().getName() + "; saveFile start, content length: " + response.contentLength());
        try {
            try {
                int readLen;
...
//                long end = recordBuffer.getLong(startIndex + 8);
                ...
                while ((readLen = inStream.read(buffer)) != -1 && !emitter.isCancelled()) {
                    ...
                    if(start - oldStart > 100000L){
                        Utils.log("Thread: " + Thread.currentThread().getName() + "; saveLenRead: " + start);
                        oldStart = start;
                    }
    ...
    private void prepareFile(File saveFile, long fileLength) throws IOException {
        ...
        try {
            ...
            if (fileLength != -1) {
                ...
                Utils.log(CHUNKED_DOWNLOAD_HINT);
                //Chunked 下载, 无需设置文件大小.
            }
        } finally {
