...
import cry.com.download.zlc.season.rxdownload2.function.DownloadApi;
import cry.com.download.zlc.season.rxdownload2.function.FileHelper;
...
import static cry.com.download.zlc.season.rxdownload2.entity.DownloadFlag.*;
import static cry.com.download.zlc.season.rxdownload2.function.Constant.CACHE;
import static cry.com.download.zlc.season.rxdownload2.function.Constant.RANGE_DOWNLOAD_STARTED;
...
public class TemporaryRecord {
    ...
    public void init(int maxThreads, int maxRetryCount, String defaultSavePath,
        ...
        String realSavePath;
        if (Utils.empty(bean.getSavePath())) {
        ...
        String[] paths = Utils.getPaths(bean.getSaveName(), realSavePath);
        filePath = paths[0];
        tempPath = paths[1];
        lmfPath = paths[2];
    }
        ...
        return Flowable
                ...
                .flatMap(new Function<DownloadRange, Publisher<Response<ResponseBody>>>() {
                    ...
                    public Publisher<Response<ResponseBody>> apply(DownloadRange range)
                            throws Exception {
                        Utils.log("Thread: " + Thread.currentThread().getName() + "; " + RANGE_DOWNLOAD_STARTED, index, range.start, range.end);
